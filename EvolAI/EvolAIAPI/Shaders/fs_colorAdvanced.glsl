...
main()
{
 ...
 for(int i = 0; i < 5; i++){
  ...
  // Ambient lighting
  outputColor = outputColor + outputColor * light_ambient;

  // Diffuse lighting
  float lambertmaterial_diffuse = max(dot(n, lightvec), 0.0);

  // Spotlight, limit light to specific angle
  outputColor = outputColor + (light_diffuse * outputColor * lambertmaterial_diffuse);

  // Specular lighting
  vec3 reflectionvec = normalize(reflect(-lightvec, v_norm));
  vec3 viewvec = normalize(vec3(inverse(view) * vec4(0,0,0,1)) - v_pos); 
  
  float material_specularreflection = max(dot(v_norm, lightvec), 0.0) * pow(max(dot(reflectionvec, viewvec), 0.0), 1);
  
  // Spotlight, specular reflections are also limited by angle
  outputColor = outputColor + vec4(lights[i].color, 0.0) * material_specularreflection;
 }
